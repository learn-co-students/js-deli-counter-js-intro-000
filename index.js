...
var katzDeliLine = []; //create array for the deli line. unecessary per instructions but nice for my brain right now

function takeANumber(katzDeliLine, name) {
  katzDeliLine.push(name) //adds a person to katzDeliLine array
...
function nowServing(katzDeliLine) {
  if (katzDeliLine.length < 1){ // if nobody is in line
    return "There is nobody waiting to be served!" // tell the customers there is no one in line
  } else { // if not... 
    return `Currently serving ${katzDeliLine.shift()}.`// tell the customers who is 1st in line
  }
}
...
function currentLine(katzDeli) {
  ...
  for (let i = 0; i < katzDeli.length; i++) {
    whoIsInLine.push(` `+[i+1]+`. `  + katzDeli[i])
  }  
  return(`The line is currently:` + whoIsInLine);
 }
